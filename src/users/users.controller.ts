...
export class UsersController {
	...
	async addFriends(
		...
		const listRequest = await this.usersService.addFriends(
			req.user.userId,
			body,
		);
	...
	async acceptFriend(
		...
		const listFriends = await this.usersService.acceptFriend(
			req.user.userId,
			body,
		);
	...
	async declineFriend(
		...
		const listFriends = await this.usersService.declineFriend(
			req.user.userId,
			body,
		);
		return res.status(201).json({ status: 'ok', listFriends });
	...
	async deleteFriend(
		...
		const listFriends = await this.usersService.deleteFriend(
			req.user.userId,
			body,
		);
		return res.status(201).json({ status: 'ok', listFriends });
