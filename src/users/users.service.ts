...
export class UsersService {
	...
	async findOneUser(email: string) {
		const getOneUser = await this.usersRepository.findOne({
			'profile.email': email,
		});
		return getOneUser;
	}
	...
	async addFriends(userId: string, body: AddFriendDTO) {
		try {
			const findFriend = await this.usersRepository.findOne({
			...
			const sendedRequest =
				await this.usersRepository.findOneAndUpdateUser(
					{ _id: new ObjectId(userId) },
					{ $push: { sended_request: body.userId } },
			...
			await this.usersRepository.findOneAndUpdateUser(
				{ _id: new ObjectId(body.userId) },
				{ $push: { received_requests: pendingRequest } },
			);
	...
	async acceptFriend(userId: string, body: AcceptFriendDTO) {
		const acceptFriend = await this.usersRepository.findOneAndUpdateUser(
			{
				_id: new ObjectId(userId),
				'received_requests.userId': body.userId,
			},
			{
				$set: { 'received_requests.$.status': RequestStatus.ACCEPTED },
				$push: { friends: new ObjectId(body.userId) },
			},
			{
				returnDocument: 'after',
				projection: { _id: 1, 'profile.password': 0 },
