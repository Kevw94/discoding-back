...
export class ServersController {
	...
	async getServers(@Req() req: JwtRequest, @Res() res: Response) {
		const serversMe = await this.serversService.getAllServersMe(req.user.userId)
		return res.status(200).json({ status: 'ok', servers: serversMe});
	}

	@Post('create')
	async createServer(@Req() req: JwtRequest, @Res() res: Response, @Body() body: CreateServerDTO) {
		const newServer = await this.serversService.createServer(req.user.userId, body)
